...
public static class Utils
{
    ...
    // This is the same as the above function, but it doesn't check the top of the screen.
    public static bool IsGameObjectOutOfScreenBoundsNotTop(GameObject gameObject, Camera camera)
    {
        // Get the bounds of the screen in world space.
        float screenAspect = (float)Screen.width / (float)Screen.height;
        float cameraHeight = camera.orthographicSize * 2;
        ...
        // Check if the gameobject's position is outside of the screen bounds.
        Vector2 gameObjectPos = camera.WorldToViewportPoint(gameObject.transform.position);
        return gameObjectPos.x < 0f || gameObjectPos.x > 1f || gameObjectPos.y < 0f;
    }
}
